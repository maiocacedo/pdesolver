...
A Tabela~\ref{tab:resultados} apresenta os seis problemas de referência com solução fechada ou de referência de alta ordem: calor 1D e 2D, advecção-difusão, Burgers viscosa, equação da onda 1D (reformulada como sistema de primeira ordem) e presa-predador (reação-difusão acoplada). As colunas \texttt{/nó} e \texttt{/vet} identificam o caminho de discretização.
...
\midrule
Equação da Onda 1D$^{\dagger}$
  & PDESolver/BDF2        & $1{,}3163\times10^{-4}$ & --- & --- \\
  & PDESolver/CN          & $\mathbf{2{,}6550\times10^{-7}}$ & --- & --- \\
  & PDESolver/RKF         & $4{,}8931\times10^{-7}$ & --- & --- \\
  & FiPy/splitting        & $2{,}7602\times10^{-4}$ & --- & --- \\
  & py-pde/LSODA          & $7{,}7100\times10^{-4}$ & --- & --- \\
\midrule
Presa-Predador$^{\ddagger}$
  & PDESolver/BDF2        & $2{,}2260\times10^{-4}$ & --- & --- \\
  & PDESolver/CN          & $2{,}2230\times10^{-4}$ & --- & --- \\
  & PDESolver/RKF         & $2{,}2210\times10^{-4}$ & --- & --- \\
  & FiPy/Picard-CN        & $1{,}4650\times10^{-3}$ & --- & --- \\
  & py-pde/LSODA          & $\mathbf{2{,}1010\times10^{-4}}$ & --- & --- \\
\bottomrule
\end{tabular}}

\vspace{2pt}
{\footnotesize $^{\dagger}$ RMSE sobre $U$ contra a solução analítica $U=\sin(\pi x)\cos(\pi c t)$.
$^{\ddagger}$ RMSE médio $(\mathrm{RMSE}_U+\mathrm{RMSE}_V)/2$ contra referência Radau em malha fina.
Tempos de Onda 1D e Presa-Predador (---) pendentes de re-medição na máquina de referência (i7-14700K); os valores de RMSE foram verificados. O RMSE do py-pde na Onda 1D depende da versão da biblioteca (py-pde~0.56.1).}
\end{table}

% RMSE de Onda 1D e Presa-Predador verificado (NT=200, dt=5e-3, config declarada):
% PDESolver e FiPy reproduzem o artigo original; py-pde/LSODA reproduz no predador
% (2,10e-4) e depende de versao na onda. Scripts corrigidos em benchmarks/
% (benchmark_onda.py, benchmark_predador.py: NT=200, N_RUNS=100).
% FALTA: re-medir os TEMPOS dos 6 casos juntos na maquina de referencia (i7-14700K)
% e preencher as colunas "---" das duas linhas novas.
